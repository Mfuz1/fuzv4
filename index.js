...
const vcard = 'BEGIN:VCARD\n' 
            + 'VERSION:3.0\n' 
            + 'FN:MFUZ\n' 
            + 'ORG: Pengembang FuzBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=62895619379748:+62 819-5336-0563\n' 
            + 'END:VCARD' 
blocked = []   
prefix = '/'
limitawal = 300
...
/******** OWNER NUMBER**********/
const ownerNumber = ["62895619379748@s.whatsapp.net"] 
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `akhirnya beban group berkurang 𝟭,bye bye🥳 @${num.split('@')[0]} kalau balik nitip seblak buat empuss yaa`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
            ...
            case 'bitly':
               ...
               case 'help': 
                ...
                case 'mfuz':
                  ...
                  client.sendMessage(from, 'Tuh nomer owner ku MFUZ >_<, jangan spam atau ku block kamu',MessageType.text, { quoted: mek} )
					break    
				case 'leaderboard':
				case 'lb':
				case 'rangking':
               ...
               //if (anu.error) return reply(anu.error)
                 infomp3 = `「 *PLAY MP3 BYLARGANSS* 」\n*• Judul:* ${anu.result.title}\n*• Source:* ${anu.result.source}\n*• Ukuran:* ${anu.result.size}\n\n*TUNGGU SEBENTAR LAGI DIKIRIM MOHON JANGAN SPAM YA SAYANGn\AUDIO BY LARGANS YAkk!!*`
...
• Limit: ${limitawal-updated.limit}`
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'simi':
				case 'empus':
					if (args.length < 1) return reply('Hadirrr,mau ngomong apa ama aku?')
					teks = body.slice(5)
